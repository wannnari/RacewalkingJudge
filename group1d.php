...
if($mode === 'delete'){
        try {
            ...
            $time = date("H:i");
            if($_POST['pF'] == 1){
                $sql = "UPDATE judge1 SET YellowBent = NULL  WHERE id = :tNum ";  
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();    
            }else if($_POST['pF'] == 2){
                $sql = "UPDATE judge1 SET YellowLoss = NULL WHERE id = :tNum ";
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();
            }else if($_POST['pF'] == 3){
                $sql = "UPDATE judge1 SET RedCard = NULL, RedTime = NULL WHERE id = :tNum ";
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();
            }else if($_POST['pF'] == 4){
