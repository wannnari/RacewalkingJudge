...
if($mode === 'update'){
        try {
            ...
            $time = date("H:i:s");
            if($_POST['pF'] == 1){
                $sql = "UPDATE judge4 SET YellowBent = :time WHERE id = :tNum AND YellowBent IS NULL AND RedCard IS NULL;";  
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':time',$time,PDO::PARAM_STR);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();  
            }else if($_POST['pF'] == 2){
                $sql = "UPDATE judge4 SET YellowLoss = :time WHERE id = :tNum AND YellowLoss IS NULL AND RedCard IS NULL;";
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':time',$time,PDO::PARAM_STR);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();  
            }else if($_POST['pF'] == 3){
                $sql = "UPDATE judge4 SET RedCard = '>', RedTime = :time WHERE id = :tNum AND RedCard IS NULL;";
                $stmh = $pdo->prepare($sql);
                $stmh->bindValue(':time',$time,PDO::PARAM_STR);
                $stmh->bindValue(':tNum',$tNum,PDO::PARAM_STR);
                $stmh->execute();  
...
try {
    $pdo = new PDO($dsn, $user, $password);
    $table_data = array();
    $sql = "SELECT * FROM judge4 ORDER BY ord;";
    $stmh = $pdo->prepare($sql);
    $stmh->execute();
} catch (PDOException $e) {
    echo 'データベースにアクセスできません!' . $e->getMessage();
